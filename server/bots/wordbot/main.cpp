...
int main(int argc, char* argv[])
{
    ...
    threads.join_all();*/
    PyObject *pName, *pModule, *pDict, *pFunc, *pValue, *pArg, *pArgs;
    Py_Initialize();
    pName = PyUnicode_DecodeFSDefault("w2v");
    pArg = PyBytes_FromString("qwer");
    pArgs = PyTuple_New(1);
    PyTuple_SetItem(pArgs, 0, PyBytes_FromString("Игорь олег футбол"));
    pModule = PyImport_Import(pName);
    if(pModule!=NULL){
        pDict = PyModule_GetDict(pModule);
        pFunc = PyDict_GetItemString(pDict, "get_not_mach");
